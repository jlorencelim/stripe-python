...
class TestSubscriptionScheduleRevision(object):
    def construct_resource(self):
        ...
        return stripe.SubscriptionScheduleRevision.construct_from(revision_dict, stripe.api_key)

    def test_has_instance_url(self, request_mock):
        resource = self.construct_resource()
        assert (
            resource.instance_url()
            == "/v1/subscription_schedules/sub_sched_123/revisions/%s" % TEST_RESOURCE_ID
        )
